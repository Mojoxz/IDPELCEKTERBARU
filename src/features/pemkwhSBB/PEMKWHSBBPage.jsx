...
import { processPEMKWHSBB, exportToExcelWithFormatting } from '../../utils/pemkwhlebih10kSBB';
...
const PEMKWHSBBPage = () => {
  ...
  const handleProcess = async () => {
    ...
    try {
      ...
      const processedResults = processPEMKWHSBB(wb);
      ...
      if (totalSheets === 0) {
        setAlert({
          type: 'info',
          message: 'Tidak ada data dengan Kolom K > 10000 ditemukan'
        });
        setResults(null);
      } else {
  ...
  const handleDownload = () => {
    if (results && Object.keys(results).length > 0) {
      const filename = `PEMKWH_SBB_Kolom_K_${Date.now()}.xlsx`;
          ...
          <div className="flex items-center mb-6">
            ...
            <div>
              <h1 className="text-3xl font-bold text-gray-900 mb-2">
                Filter Kolom K {">"} 10000 (SBB)
              </h1>
              <p className="text-gray-600 text-lg">
                Filter data dengan Kolom K lebih dari 10000, Header di baris 9, Kolom A–P
              </p>
            </div>
          </div>
          ...
          <div className="bg-gray-50 rounded-xl p-4 border border-gray-100">
            <div className="flex items-center">
              ...
              <p className="text-gray-600 text-sm">
                Upload file Excel SBB dan sistem akan memfilter data dengan rumus Kolom K {">"} 10000 dengan highlight kuning
              </p>
            </div>
          </div>
        </div>
            ...
            {results && Object.keys(results).length > 0 && (
              <div className="bg-green-50 rounded-xl p-6 border border-green-200 animate-slide-up">
                ...
                <p className="text-xs text-gray-600 mt-2 text-center">
                  File akan di-highlight kuning untuk baris dengan K {">"} 10000
                </p>
              </div>
            )}
            ...
            {results && Object.keys(results).length > 0 && (
              <div className="space-y-6">
                {Object.keys(results).map((sheetName, idx) => {
                  const { headers, data, columnKIndex } = results[sheetName];
                                ...
                                {headers.map((header, idx) => (
                                  <th 
                                    ...
                                    className={`px-4 py-3 text-left text-xs font-medium uppercase tracking-wider ${
                                      idx === columnKIndex ? 'bg-yellow-50 text-yellow-700' : 'text-gray-500'
                                    }`}
                                  >
                                    {header}
                                  </th>
                                  ...
                                  {headers.map((header, colIdx) => (
                                    <td 
                                      ...
                                      className={`px-4 py-3 whitespace-nowrap text-sm ${
                                        colIdx === columnKIndex ? 'bg-yellow-50 font-semibold text-yellow-900' : 'text-gray-900'
                                      }`}
                                    >
            ...
            {!file && !loading && (
              <div className="bg-white rounded-xl shadow-sm p-12 text-center">
                ...
                <p className="text-gray-600 mb-6">
                  Upload file Excel SBB untuk memulai pemrosesan data
                </p>
