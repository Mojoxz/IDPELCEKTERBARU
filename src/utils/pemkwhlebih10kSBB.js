...
/**
 * Proses file Excel untuk filter kolom K > 10000
 * @param {Object} workbook - XLSX workbook object
 * @returns {Object} - Object berisi hasil filter per sheet
 */
export const processPEMKWHSBB = (workbook) => {
  const results = {};
  ...
  workbook.SheetNames.forEach(sheetName => {
    try {
      ...
      // Pastikan ada minimal 11 kolom (untuk kolom K di index 10)
      if (headers.length < 11) {
        console.log(`Sheet '${sheetName}' tidak memiliki kolom K`);
        return;
      }
      
      const colKName = headers[10]; // Kolom K = index 10
      ...
      const filtered = data.filter(row => {
        const colKValue = parseFloat(row[colKName]);
        return !isNaN(colKValue) && colKValue > 10000;
      });
      
      if (filtered.length === 0) {
        console.log(`Sheet '${sheetName}' tidak ada data K>10000`);
        return;
      }
      
      // Sort descending berdasarkan kolom K
      filtered.sort((a, b) => {
        const valA = parseFloat(a[colKName]) || 0;
        const valB = parseFloat(b[colKName]) || 0;
        return valB - valA;
      });
      ...
      results[sheetName] = {
        headers,
        data: filtered,
        columnKIndex: 10 // Untuk highlighting
      };
      
    } catch (error) {
      console.error(`Error pada sheet '${sheetName}':`, error);
    }
  });
...
export const exportToExcelWithFormatting = (results, filename) => {
  ...
  Object.keys(results).forEach(sheetName => {
    ...
    // Terapkan styling untuk kolom K (highlight kuning)
    ...
    for (let row = 1; row <= range.e.r; row++) { // Mulai dari baris 1 (setelah header)
      for (let col = 0; col <= range.e.c; col++) {
        ...
        // Highlight baris jika kolom K > 10000
        const colKAddress = XLSX.utils.encode_cell({ r: row, c: 10 });
        const colKCell = ws[colKAddress];
        if (colKCell && parseFloat(colKCell.v) > 10000) {
